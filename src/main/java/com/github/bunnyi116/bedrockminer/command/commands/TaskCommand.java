...
public class TaskCommand extends CommandBase {
                ...
                .then(literal("limitMax")
                        .then(argument("int", IntegerArgumentType.integer(1, 10))
                                .executes(context -> {
                                    ...
                                    MessageUtils.addMessage(Text.literal("任务同步执行限制最大值已设置为: " + limitMax));
                ...
                .then(literal("remove")
                        .then(argument("name", StringArgumentType.string())
                                ...
                                .executes(context -> {
                                    ...
                                    for (final TaskRegion item : Config.getInstance().ranges) {
                                    ...
                                    if (range == null) {
                                        for (final TaskRegion item : TaskManager.getInstance().getPendingRegionTasks()) {
                                            if (item.name.equals(name)) {
                                                range = item;
                                                break;
                                            }
    ...
    private int addRegionTask(CommandContext<FabricClientCommandSource> context, boolean isConfig) {
        final String name = StringArgumentType.getString(context, "name");
        final BlockPos blockPos1 = BlockPosArgumentType.getBlockPos(context, "blockPos1");
        final BlockPos blockPos2 = BlockPosArgumentType.getBlockPos(context, "blockPos2");
        boolean b = true;
        if (isConfig) {
            for (final TaskRegion item : ConfigManager.getInstance().getConfig().ranges) {
                if (item.name.equals(name)) {
                    b = false;
                    break;
                }
            ...
            if (b) {
                final TaskRegion range = new TaskRegion(name, BedrockMiner.world, blockPos1, blockPos2);
